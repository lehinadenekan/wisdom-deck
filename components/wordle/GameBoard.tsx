...
const GameBoard: React.FC<GameBoardProps> = ({ guesses, currentGuess, turn, solution, showTonalAccents, className }) => {
  ...
  return (
    <div className={`relative ${className || ''}`}>
      <div className="grid grid-rows-6 gap-1 sm:gap-1.5 mb-2 sm:mb-4">
            ...
            return (
              <div key={rowIndex} className="grid grid-cols-5 gap-1 sm:gap-1.5">
                {guess.map((tile, colIndex) => (
                  <div 
                    key={colIndex}
                    className={`w-12 h-12 sm:w-16 sm:h-16 border-2 flex items-center justify-center text-xl sm:text-3xl font-bold uppercase
            ...
            return (
              <div key={rowIndex} className="grid grid-cols-5 gap-1 sm:gap-1.5">
                {Array.from({ length: totalCols }).map((_, colIndex) => (
                  <div 
                    key={colIndex}
                    className="w-12 h-12 sm:w-16 sm:h-16 border-2 border-gray-600 flex items-center justify-center text-xl sm:text-3xl font-bold uppercase"
                  >
          ...
          return (
            <div key={rowIndex} className="grid grid-cols-5 gap-1 sm:gap-1.5">
              {Array.from({ length: totalCols }).map((_, colIndex) => (
                <div 
                  key={colIndex}
                  className="w-12 h-12 sm:w-16 sm:h-16 border-2 border-gray-600"
                />
              ))}
            </div>
          );
      ...
      {showTonalAccents && (
        <div className="absolute -top-8 left-0 right-0 grid grid-cols-5 gap-1.5 pointer-events-none">
          {getGraphemes(solution).map((char, index) => {
            const accent = getDiacritic(char);
            return (
              <div key={index} className="w-12 h-12 sm:w-16 sm:h-16 flex items-start justify-center">
                <span className="text-xl sm:text-3xl text-yellow-400 font-semibold">
                  {accent}
                </span>
              </div>
            );
