...
// Improved matching function - since ALL words should be in dictionary
function findBestMatch(targetWord, dictionaryMap) {
  ...
  for (const [dictWord, entry] of dictionaryMap) {
    if (calculateSimilarity(normalizedTarget, dictWord) > 0.7) {
      return entry;
    }
  }
    ...
    while (hasMore) {
      try {
        ...
        const { data: records, error } = await supabase
          .from(table)
          .select('id, word, english_translation, example_sentence')
        ...
        for (const record of records) {
          try {
            ...
            if (dictionaryEntry && dictionaryEntry.english_translation) {
              ...
              // Only update part_of_speech if we have it and it's different
              if (dictionaryEntry.part_of_speech && dictionaryEntry.part_of_speech.trim() !== '') {
                updateData.part_of_speech = dictionaryEntry.part_of_speech;
              }
              
              // Only update example_sentence if we have it and it's not null
              if (dictionaryEntry.example_sentence && dictionaryEntry.example_sentence.trim() !== '') {
                updateData.example_sentence = dictionaryEntry.example_sentence;
              }
...
async function main() {
  try {
    console.log('🚀 Starting Final Supabase Translation Update...');
